...
%--------------------------------------------------------------------------
    ...
    frameAdjustmentFlag =optns{'frameAdjustmentFlag'} ;% **Complete Code**     % Matlab's base_link does not match ros. Need adjustment.
    toolAdjustmentFlag  = optns{'toolAdjustmentFlag'};% **Complete Code**      % We have fingers but have not adjusted IKs for them. Transform from wrist to tips. 
    z_offset = optns{'z_offset'}; % **Complete Code**                           % Instead of grasping model (i.e. can) from CoM, grasp from top.
    ...
    W_T_R = get_model_pose('robot', optns);% **Complete Code**
    W_T_M = get_model_pose(model_name,optns); % **Complete Code**
    ...
    mat_R_T_M = inv(mat_W_T_R) *mat_W_T_M; % **Complete Code** % OK to use inv for homogeneous transformation

    % Offset along +z_base_link to simulate knowing height of top of can.
    mat_R_T_M(3,4) =mat_R_T_M(3,4) + z_offset; % **Complete Code**
    ...
    try
        current_pose = getTransform(r.tftree,base,end_effector, rostime('now'), 'Timeout',r.tf_listening_time);% **Complete Code**
    catch
        % Try again
        current_pose = getTransform(r.tftree,base,end_effector, rostime('now'), 'Timeout',r.tf_listening_time); % **Complete Code**   
    end
