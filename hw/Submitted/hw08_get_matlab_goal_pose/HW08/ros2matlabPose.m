function matlab_pose = ros2matlabPose(p,frameAdjustmentFlag,toolAdjustmentFlag,optns)
    ...
    if ~frameAdjustmentFlag 
        ...
        % gazebo_msgs/GetModelStateResponse: extract an [x,y,z] position
        % and a matlab UnitQuaternion q based on ROS [W,X,Y,Z] data.
        % Specific to the ROS type for the message Pose Position and Orientation
        if strcmp(p.MessageType, 'gazebo_msgs/GetModelStateResponse')
            pos =[p.Pose.Position.X, ...
                p.Pose.Position.Y, ...
                p.Pose.Position.Z]; % **Complete Code** 
    
            q = quat2rotm([p.Pose.Orientation.W, ...
                p.Pose.Orientation.X ,...
                p.Pose.Orientation.Y, ...
                p.Pose.Orientation.Z]); %**Complete Code** 

        % geometry_msgs/TransformStamped: extract an [x,y,z] position and a
        % matlab UnitQuaternion q based on ROS [W,X,Y,Z] data. Specific to
        % the ROS type for the message Pose Translation and Rotation
        elseif strcmp(p.MessageType, 'geometry_msgs/TransformStamped')
            pos =  [p.Transform.Translation.X, ...
                    p.Transform.Translation.Y,... 
                    p.Transform.Translation.Z]; % **Complete Code** 
    
            q = quat2rotmpq([p.Transform.Rotation.W, ... 
                p.Transform.Rotation.X, ...
                p.Transform.Rotation.Y, ...
                p.Transform.Rotation.Z]);% **Complete Code** 
        end
    
    % Matlab axes Adjustment
    ...
    % ros_x=>mat_y, ros_y=>-mat_x, z=z. Or mat_x = -ros_y, mat_y=ros_x mat_z=ros_z + adjustment
        ...
        if strcmp(p.MessageType, 'gazebo_msgs/GetModelStateResponse')
            ...
            q = quat2rotm([p.Pose.Orientation.W, ...
                p.Pose.Orientation.X ,...
                p.Pose.Orientation.Y, ...
                p.Pose.Orientation.Z])*rotz(-90, 'deg'); ;
        ...
        elseif strcmp(p.MessageType, 'geometry_msgs/TransformStamped')
             pos =  [p.Transform.Translation.X,...
                 p.Transform.Translation.Y, ...
                 p.Transform.Translation.Z + endEffectorAdjustment]; ; % **Complete Code** 
    
            q = quat2rotm([p.Transform.Rotation.W, ... 
                p.Transform.Rotation.X, ...
                p.Transform.Rotation.Y, ...
                p.Transform.Rotation.Z]) *rotz(-90, 'deg') ;% **Complete Code** 
        end
    end

    % Build matlab pose
    matlab_pose = transl(pos) * rotm2tform(q);
end